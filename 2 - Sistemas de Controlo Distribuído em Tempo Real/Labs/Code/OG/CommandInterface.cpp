...
/**
 * Handle emergency shutdown command (e)
 *
 * @param tokens Command tokens
 * @param numTokens Number of tokens
 * @return true if command was handled, false otherwise
 */
static bool handleEmergencyShutdown(char tokens[][TOKEN_MAX_LENGTH], int numTokens)
{
    // Basic syntax check - we expect "e" or "e [node ID]"
    if (strcmp(tokens[0], "e") != 0)
    {
        return false;
    }

    uint8_t targetNode = CAN_ADDR_BROADCAST; // Default to broadcast
    bool isBroadcast = true;

    // Check if a target node was specified
    if (numTokens >= 2)
    {
        int nodeId;
        if (!parseIntParam(tokens[1], &nodeId) || nodeId < 0 || nodeId > 63)
        {
            Serial.println("err: Invalid node ID (0-63)");
            return true;
        }
        targetNode = (uint8_t)nodeId;
        isBroadcast = (targetNode == 0);
    }

    // Check if we need to forward this command
    if (shouldForwardCommand(targetNode))
    {
        // Forward shutdown command to the specified node
        if (sendControlCommand(targetNode, CAN_CTRL_EMERGENCY_SHUTDOWN, 0.0f))
        {
            Serial.println("ack: Emergency shutdown command sent");
        }
        else
        {
            Serial.println("err: Failed to send emergency shutdown command");
        }
        return true;
    }

    // Handle local shutdown or broadcast
    if (isBroadcast)
    {
        // Send broadcast notification before shutting down
        sendControlCommand(CAN_ADDR_BROADCAST, CAN_CTRL_EMERGENCY_SHUTDOWN, 0.0f);
        Serial.println("Broadcasting emergency shutdown");
    }

    // Perform local shutdown
    Serial.println("Executing emergency shutdown...");
    ...
    Serial.println("Node shutdown complete. Restart required to resume operation.");
    return true;
}
...
static void processCommandLine(const char *cmdLine)
{
    ...
    else if (handleEmergencyShutdown(tokens, numTokens))
    {
        // Command handled by emergency shutdown handler
        return;
    }
    else if (handleLEDCommands(tokens, numTokens))
    {
        // Command handled by LED commands handler
        return;
    }
