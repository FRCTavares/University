...
static bool handleSystemStateCommands(char tokens[][TOKEN_MAX_LENGTH], int numTokens) {
    ...
    return true;
        ...
        // "R" command initiates a full automatic calibration sequence
        Serial.println("Initiating automatic calibration sequence");
        
        // Start calibration as master
        if (startCalibration()) {
        Serial.println("Calibration initiated, process will complete automatically");
        Serial.println("ack");
        } else {
