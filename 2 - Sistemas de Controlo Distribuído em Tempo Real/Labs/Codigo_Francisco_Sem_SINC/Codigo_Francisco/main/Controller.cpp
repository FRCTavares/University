...
void initLEDDriver() {
  ...
  node.dutyCycle = 0.0; // Use the global duty cycle variable
}
...
void setLEDDutyCycle(float newDutyCycle) {
    ...
    int pwmValue = (int)(newDutyCycle * PWM_MAX);
    analogWrite(LED_PIN, pwmValue);
    
    // Update the global duty cycle
    node.dutyCycle = newDutyCycle;
}
...
/**
 * Set LED brightness using direct PWM value (0 to PWM_MAX)
...
void setLEDPWMValue(int pwmValue) {
    ...
    node.dutyCycle = (float)pwmValue / PWM_MAX;
...
void smoothTransition(float targetDutyCycle, int transitionTimeMs) {
    ...
    float startDutyCycle = node.dutyCycle;
