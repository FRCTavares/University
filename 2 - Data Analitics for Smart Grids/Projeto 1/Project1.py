...
def compare_temporal_correlation_different_noise():
    ...
    noise_results = {
        'no_smoothing': {model: {'RMSE': [], 'MAE': []} for model in 
                         ['Original', 'Topologia (edge-reduced)', 
                          'Quadrático (squares-only)', 
                          'Injeções Próximas (squares-reduced)']},
        'smoothing': {model: {'RMSE': [], 'MAE': []} for model in 
                      ['Original', 'Topologia (edge-reduced)', 
                       'Quadrático (squares-only)', 
                       'Injeções Próximas (squares-reduced)']}
    }
    ...
    model_params = {model: 0 for model in ['Original', 'Topologia (edge-reduced)', 
                                          'Quadrático (squares-only)', 
                                          'Injeções Próximas (squares-reduced)']}
    ...
    model_params['Original'] = X_orig_train.shape[1]
    model_params['Topologia (edge-reduced)'] = X_topo_train.shape[1]
    model_params['Quadrático (squares-only)'] = X_sq_train.shape[1]
    model_params['Injeções Próximas (squares-reduced)'] = X_nearby_train.shape[1]
    ...
    for noise_level in noise_levels:
        ...
        mae_original = mean_absolute_error(PL2_test_true, pred_test_original)
        
        noise_results['no_smoothing']['Original']['RMSE'].append(rmse_original)
        noise_results['no_smoothing']['Original']['MAE'].append(mae_original)
        ...
        mae_topo = mean_absolute_error(PL2_test_true, pred_test_topo)
        
        noise_results['no_smoothing']['Topologia (edge-reduced)']['RMSE'].append(rmse_topo)
        noise_results['no_smoothing']['Topologia (edge-reduced)']['MAE'].append(mae_topo)
        ...
        mae_sq = mean_absolute_error(PL2_test_true, pred_test_sq)
        
        noise_results['no_smoothing']['Quadrático (squares-only)']['RMSE'].append(rmse_sq)
        noise_results['no_smoothing']['Quadrático (squares-only)']['MAE'].append(mae_sq)
        ...
        mae_nearby = mean_absolute_error(PL2_test_true, pred_test_nearby)
        
        noise_results['no_smoothing']['Injeções Próximas (squares-reduced)']['RMSE'].append(rmse_nearby)
        noise_results['no_smoothing']['Injeções Próximas (squares-reduced)']['MAE'].append(mae_nearby)
        ...
        mae_original_smooth = mean_absolute_error(PL2_test_true_smooth, pred_test_original_smooth)
        
        noise_results['smoothing']['Original']['RMSE'].append(rmse_original_smooth)
        noise_results['smoothing']['Original']['MAE'].append(mae_original_smooth)
        ...
        mae_topo_smooth = mean_absolute_error(PL2_test_true_smooth, pred_test_topo_smooth)
        
        noise_results['smoothing']['Topologia (edge-reduced)']['RMSE'].append(rmse_topo_smooth)
        noise_results['smoothing']['Topologia (edge-reduced)']['MAE'].append(mae_topo_smooth)
        ...
        mae_sq_smooth = mean_absolute_error(PL2_test_true_smooth, pred_test_sq_smooth)
        
        noise_results['smoothing']['Quadrático (squares-only)']['RMSE'].append(rmse_sq_smooth)
        noise_results['smoothing']['Quadrático (squares-only)']['MAE'].append(mae_sq_smooth)
        ...
        mae_nearby_smooth = mean_absolute_error(PL2_test_true_smooth, pred_test_nearby_smooth)
        
        noise_results['smoothing']['Injeções Próximas (squares-reduced)']['RMSE'].append(rmse_nearby_smooth)
        noise_results['smoothing']['Injeções Próximas (squares-reduced)']['MAE'].append(mae_nearby_smooth)
...
def compare_temporal_correlation():
    ...
    # Store results
    results = {
        'no_smoothing': {model: {'RMSE': 0, 'MAE': 0} for model in 
                       ['Original', 'Topologia (edge-reduced)', 
                        'Quadrático (squares-only)', 
                        'Injeções Próximas (squares-reduced)']},
        'smoothing': {model: {'RMSE': 0, 'MAE': 0} for model in 
                     ['Original', 'Topologia (edge-reduced)', 
                      'Quadrático (squares-only)', 
                      'Injeções Próximas (squares-reduced)']}
    }
    ...
    mae_orig = mean_absolute_error(PL2_test_true, pred_test_orig)
    results['no_smoothing']['Original']['RMSE'] = rmse_orig
    results['no_smoothing']['Original']['MAE'] = mae_orig
    ...
    rmse_topo = np.sqrt(mean_squared_error(PL2_test_true, pred_test_topo))
    mae_topo = mean_absolute_error(PL2_test_true, pred_test_topo)
    results['no_smoothing']['Topologia (edge-reduced)']['RMSE'] = rmse_topo
    results['no_smoothing']['Topologia (edge-reduced)']['MAE'] = mae_topo
    ...
    rmse_sq = np.sqrt(mean_squared_error(PL2_test_true, pred_test_sq))
    mae_sq = mean_absolute_error(PL2_test_true, pred_test_sq)
    results['no_smoothing']['Quadrático (squares-only)']['RMSE'] = rmse_sq
    results['no_smoothing']['Quadrático (squares-only)']['MAE'] = mae_sq
    ...
    rmse_nearby = np.sqrt(mean_squared_error(PL2_test_true, pred_test_nearby))
    mae_nearby = mean_absolute_error(PL2_test_true, pred_test_nearby)
    results['no_smoothing']['Injeções Próximas (squares-reduced)']['RMSE'] = rmse_nearby
    results['no_smoothing']['Injeções Próximas (squares-reduced)']['MAE'] = mae_nearby
    ...
    mae_orig_smooth = mean_absolute_error(PL2_test_true_smooth, pred_test_orig_smooth)
    results['smoothing']['Original']['RMSE'] = rmse_orig_smooth
    results['smoothing']['Original']['MAE'] = mae_orig_smooth
    ...
    rmse_topo_smooth = np.sqrt(mean_squared_error(PL2_test_true_smooth, pred_test_topo_smooth))
    mae_topo_smooth = mean_absolute_error(PL2_test_true_smooth, pred_test_topo_smooth)
    results['smoothing']['Topologia (edge-reduced)']['RMSE'] = rmse_topo_smooth
    results['smoothing']['Topologia (edge-reduced)']['MAE'] = mae_topo_smooth
    ...
    rmse_sq_smooth = np.sqrt(mean_squared_error(PL2_test_true_smooth, pred_test_sq_smooth))
    mae_sq_smooth = mean_absolute_error(PL2_test_true_smooth, pred_test_sq_smooth)
    results['smoothing']['Quadrático (squares-only)']['RMSE'] = rmse_sq_smooth
    results['smoothing']['Quadrático (squares-only)']['MAE'] = mae_sq_smooth
    ...
    rmse_nearby_smooth = np.sqrt(mean_squared_error(PL2_test_true_smooth, pred_test_nearby_smooth))
    mae_nearby_smooth = mean_absolute_error(PL2_test_true_smooth, pred_test_nearby_smooth)
    results['smoothing']['Injeções Próximas (squares-reduced)']['RMSE'] = rmse_nearby_smooth
    results['smoothing']['Injeções Próximas (squares-reduced)']['MAE'] = mae_nearby_smooth
    ...
    rects1 = ax[0].bar(x - width/2, rmse_no_smoothing, width, label='Sem Correlação Temporal')
    rects2 = ax[0].bar(x + width/2, rmse_smoothing, width, label='Com Correlação Temporal')
    
    ax[0].set_ylabel('RMSE')
    ax[0].set_title('Erro (RMSE) Com e Sem Correlação Temporal')
    ax[0].set_xticks(x)
    ax[0].set_xticklabels(models, rotation=45, ha='right')
    ax[0].legend()
    
    # MAE plot
    mae_no_smoothing = [results['no_smoothing'][model]['MAE'] for model in models]
    mae_smoothing = [results['smoothing'][model]['MAE'] for model in models]
    
    rects3 = ax[1].bar(x - width/2, mae_no_smoothing, width, label='Sem Correlação Temporal')
    rects4 = ax[1].bar(x + width/2, mae_smoothing, width, label='Com Correlação Temporal')
    
    ax[1].set_ylabel('MAE')
    ax[1].set_title('Erro (MAE) Com e Sem Correlação Temporal')
    ax[1].set_xticks(x)
    ...
    for i in range(4):
        row, col = i // 2, i % 2
        time_range = 100  # Just plot a portion for clarity
        ax[row, col].plot(P[:time_range, i], label='Original')
        ax[row, col].plot(P_smooth[:time_range, i], label='Com Correlação Temporal')
    ...
    print("\nMelhoria percentual com correlação temporal:")
    print("-" * 70)
    print("Modelo".ljust(30) + " | " + "Melhoria RMSE (%)".ljust(20) + " | " + "Melhoria MAE (%)".ljust(20))
    print("-" * 70)
    
    for model in models:
        rmse_improvement = 100 * (results['no_smoothing'][model]['RMSE'] - results['smoothing'][model]['RMSE']) / results['no_smoothing'][model]['RMSE']
        mae_improvement = 100 * (results['no_smoothing'][model]['MAE'] - results['smoothing'][model]['MAE']) / results['no_smoothing'][model]['MAE']
        print(f"{model.ljust(30)} | {rmse_improvement:.2f}%".ljust(20) + " | " + f"{mae_improvement:.2f}%".ljust(20))
...
def show_menu():
    ...
    print("  6. Comparar modelos com e sem correlação temporal\n")
    print("  \nSair do programa: '0'")
