...
/**
 * Compute control action with feedforward term
 * Combines standard PI control with a feedforward term for improved performance
 *
 * @param setpoint Target illuminance level (lux)
 * @param measurement Current illuminance level (lux)
 * @param feedforward Feedforward term (duty cycle 0-1)
 * @return PWM value for output (0-4095)
 */
float PIController::computeWithFeedforward(float setpoint, float measurement, float feedforward)
{
  // First compute the standard PI output
  float u_pi = compute(setpoint, measurement);

  // Convert feedforward duty cycle [0,1] to PWM value [0-4095]
  float ff_pwm = feedforward * 4095.0f;

  // Combine the PI output with the feedforward term
  float u_combined = u_pi + ff_pwm;

  // Saturate the output to valid PWM range
  return constrain(u_combined, 0.0f, 4095.0f);
}
//==========================================================================================================================================================
