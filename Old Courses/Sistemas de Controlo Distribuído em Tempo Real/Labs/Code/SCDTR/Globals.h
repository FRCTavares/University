...
// Duty cycle proposal for consensus (add in CAN message types section)
#define CAN_SENSOR_DUTY_PROPOSAL 99
...
{
  float d;    // Duty cycle
  float d_av; // Average duty cycle across nodes
  float y;    // Lagrangian multiplier
  float k;    // Coupling gain
  float c;    // Cost weight
};
...
struct ControlState
{
  ...
  float pendingDutyCycle;           // Proposed duty cycle waiting for consensus
  uint8_t consensusRound;           // Current consensus round number
  bool consensusReached;            // Whether consensus was reached
  unsigned long lastProposalTime;   // Last time a proposal was sent
  uint8_t iterationCounter;         // Counter for iterations before applying duty cycle
  ...
  float feedbackDutyCorrection; // Last duty cycle calculated by feedback controller
  bool usingSeq;

  float occupiedLuxBound;           // Configurable lower bound for occupied state illuminance
  float unoccupiedLuxBound;         // Configurable lower bound for unoccupied state illuminance
  bool usingADMM;                   // Whether ADMM control is active
  std::map<int, NodeInfo> nodeInfo; // Node information for ADMM
  float rho;                        // ADMM step size parameter
  float n;                          // Sum of squares of all gains
  float m;                          // Sum of squares minus self gain squared
  float L;                          // Illuminance lower bound
  float o;                          // External illuminance
  float cost;                       // Cost parameter
  bool equalCosts;                  // true for equal costs, false for different costs
  float energyCost;                 // Cumulative energy cost in joules
};
...
struct CommState
{
  ...
  bool useDefaultGains; // Whether to use default gains instead of calibration
};
...
struct NeighborInfo
{
  ...
  float pendingDuty;        // Proposed duty cycle (for consensus)
  bool hasProposal;         // Whether we've received a proposal from this neighbor
