...
/**
 * Send a query command to another node
 *
 * @param destNode Destination node address
 * @param queryType Type of query (variable code)
 * @param value Optional value parameter
 * @return True if message sent successfully
 */
bool sendQueryCommand(uint8_t destNode, uint8_t queryType, float value)
{
  can_frame frame;

  // Configure message ID with query type and destination
  frame.can_id = buildCANId(CAN_TYPE_QUERY, destNode);
  frame.can_dlc = 8;
