...
for k = 1:N
    ...
    [du_hat(:, k), fval(:, k)] = mpc_solve_5(dx_hat(:, k), u_ss, Du_bar, Dr, y_ss, H, R, A, B, C); % Added for P4
    if isnan(du_hat(:, k))
        break
    end
...
plot(t(1:k-1), y(1:k-1), '.', 'MarkerSize', 5)
yl = yline(Dr+y_ss, 'k--');
xlabel('Time [s]')
ylabel('y [°C]')
legend(yl, 'Reference', 'Interpreter', 'latex', 'Location', 'best')
subplot(2, 1, 2), hold on, grid on   
stairs(t(1:k-1), u(1:k-1), 'LineWidth', 2)
%yl = yline(u_ss, 'k--');
yline(0, 'r--')
yline(100, 'r--')
xlabel('Time [s]')
...
y_aux = nan(1, H);
x_aux = nan(3, H + 1);
x_aux(:, 1) = x0 + Dx_bar + x_ss;
h2 = @(x,u) A * x + B * u + c1; % apply control
T2C = @(x) C * x + c2; % read temperature

for ii = 1:H
    y_aux(:, ii) = T2C(x_aux(:, ii));
    x_aux(:, ii + 1) = h2(x_aux(:, ii), u_aux(:, ii));
end
...
subplot(2, 1, 1)
hold on
plot(t(k:k+H-1), y_aux, '.', 'MarkerSize', 10, 'color', 'red')

subplot(2, 1, 2)
hold on
stairs(t(k:k+H-1), u_aux, 'LineWidth', 2, 'color', 'red')
