% mpc_solve(x0, u_ss, Du_bar, y_ss, Dr, H, R, A, B, C, beta) 
%
% Description: Computes the gain for the Reciding Horizon (RH) optimal 
% control problem using quadprog and soft constraints, given the system 
% parameters, A, B and C, the cost function parameter R, the horizon H and 
...
function u0 = mpc_solve(x0, u_ss, Du_bar, y_ss, Dr, H, R, A, B, C, beta)   
    % Get model order
    n = size(A, 1);
    ...
    % Compute auxiliar matrices to match quadprog cost function layout
    Rs = [R * eye(H) zeros(H, H); zeros(H, H) beta * eye(H)];
    Ws = [W zeros(H, H)];
    Ms = Ws' * Ws + Rs;

    % Add lower and upper bounds for soft contraints
    ...
    [Z, ~, exitflag, ~] = quadprog(2 * Ms, (2 * x0' * PI' * Ws)', Aineq, bineq, [], [], lb, ub, x0, opts);
    ...
    % Extract the first control command
    u0 = Z(1);
end