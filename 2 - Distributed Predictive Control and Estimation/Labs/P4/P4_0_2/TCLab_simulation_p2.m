...
%% R-sweep at fixed H
H = 20;
R_values = [0.01, 0.02, 0.05, 0.1, 0.5, 1];
colorsR   = lines(length(R_values));

% preallocate
resultsR = struct('t',[],'y',[],'u',[],'Dy',[],'Du',[]);

fprintf('Running simulation for multiple R (H=20)...\n')
for idx = 1:length(R_values)
    ...
    for k = 1:N
        ...
        % build dx, dy around any set-point steps (here Dr=0)
        dy    = Dy(k);
        Du_ss = pinv(C*((eye(n)-A)\B))*0;
        dx    = Dx(:,k);
...
fprintf('R-sweep complete.\n')


%% R-sweep absolute plots (y and u)
figure('Units','normalized','Position',[0.2 0.3 0.6 0.5])

% absolute output y
subplot(2,1,1), hold on, grid on
for i = 1:length(R_values)
    plot(resultsR(i).t, resultsR(i).y, ...
         'Color', colorsR(i,:), ...
         'DisplayName', sprintf('R=%.3f',R_values(i)))
end
title('Absolute output y for H=20')
xlabel('Time [s]')
ylabel('y [°C]')
legend show
...
subplot(2,1,2), hold on, grid on
for i = 1:length(R_values)
    stairs(resultsR(i).t, resultsR(i).u, ...
           'Color', colorsR(i,:), ...
           'DisplayName', sprintf('R=%.3f',R_values(i)))
end
title('Absolute input u for H=20')
xlabel('Time [s]')
ylabel('u [%]')
legend show
